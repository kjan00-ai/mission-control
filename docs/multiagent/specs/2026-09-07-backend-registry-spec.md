...
**(a) L2 미합의 3건** — 전부 gemini 제기·claude 반박:
1. `a9592ff8`(blocker): 정책 T3 선언 vs 분류 T1 불일치 = 거버넌스 갭. → **§8 실측으로 전제가 소멸**(실제 T3). 판정 시 이 정정을 반영해 주십시오.
2. `3f999c4e`: 정책 경로 경계 검사 모호. → realpath 정규화 + 시험 4종으로 **구현에서 해소**.
3. `e4b2f73d`: `pickBackend` 라우팅 상세 미정의. → 우선순위 3단 + 미매치 null을 27개 시험으로 고정. 등재가 1건뿐이라 "복잡한 시나리오"는 현재 부재.
