...
- **대상(A2 자기보호 T3, 대표 `!`)**: `~/.ai-bootstrap/decision-policy.json` 의 `git-destructive.re` · `git-merge.re` · **`git-commit.re`(2차)**
- **영구 가드**: `~/.ai-bootstrap/gate-destructive.test.js` 74건 — 닫은 구멍이 재개방되면 시험이 깨진다
...
**② 잔여도 종결(2026-09-07 2차)**: `commit-tree`·`commit-graph`가 `git-commit`(T2)에 걸리던 것을 `commit(?!-)\b` 로 해소했다.

전 규칙 재점검 결과 다른 접두어 충돌은 없다 — `push`·`tee`·`sh`/`bash`는 실존하는 하이픈 형제가 없고, `mkfs`는 `mkfs.ext4`처럼 **점** 구분자라 `\b`가 정상 동작하며, `sudo-rs`는 실존하는 sudo 구현이라 게이트되는 것이 옳다. 읽기계열 하이픈 하위명령(`ls-files`·`rev-parse`·`for-each-ref`·`cat-file`·`diff-tree`·`show-ref`·`symbolic-ref`)은 전부 T1로 정상이다.

> 검증: 케이스 15건 PASS(해제 3·유지 8·불변 4) · 감사 고유명령 40,309건 대조 **강화 0 · 완화 0** — 실사용 영향이 없는 **순수 예방 패치**다.
...
1. **`694c55ae`** (claude 제기): 검증 코퍼스에 회피 케이스가 없어 "예상밖 0"은 과거 재현성만 증명한다. 합격 기준이 spec에 없다.
   → **✅ 종결(2026-09-07): 합격 기준을 문서가 아니라 실행 가능한 시험으로 만들었다.**
   `~/.ai-bootstrap/gate-destructive.test.js` **74건** — 파괴적 형태 48(회피 변형·옵션 접두·사이 옵션·결합 플래그 포함) + 안전 형태 24(과잉차단 감시) + 룰 귀속 2. 지적의 핵심은 *"기준이 문서에만 있으면 다음 편집이 조용히 되돌린다"* 였고, 이제 되돌리면 **시험이 깨진다.** manifest `shared`에 등재해 양 환경에 배포된다.
2. **`3e91ad19`** (claude 제기): §8의 마찰 근거(4건)와 §5의 차단 규모(46건)가 모집단이 다르다(47,510 이벤트 vs 40,108 고유명령).
   → **✅ 종결: 두 수치 병기 + 단위 명시.** 47,510 = 감사 **이벤트**(중복 포함), 40,108 = **고유 명령**. `clean -fd` 마찰 4건은 이벤트 기준이고, 차단 46건은 고유 명령 기준이다. 대표 판정(가)은 "clean -fd 4건" 위에서 내려졌고 나머지 42건은 원래 막혔어야 할 것이므로 마찰 계산에서 제외하는 것이 맞다.
