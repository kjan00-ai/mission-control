...
| `167f4774` **important (refuted)** — '논리 줄' 정의가 백슬래시 연속 등에서 모호 | **대표 판정 대기.** claude 반박: 잔여 모호성은 전부 "마스킹 포기 → 원문 유지" + `headScanText` catch → `dangerScanText` 폴백으로 **보수 방향(오탐 모달 잔존)** 으로만 귀결하므로 안전 손실이 아니다. 구현도 그 구조 그대로다 |
...
| `d446706b` **important (refuted)** — 수용 기준이 결과를 본 뒤 사후 변경됐다(§4④ '강화 0' → §7 'interactive 강화만 금지') | — | **대표 판정 대기.** 기준을 좁힌 것은 사실이다. 근거는 실측(세그먼트 22→49 실증)이고 §7 에 `interactive` 금지가 **새로 추가**되어 일부는 더 엄격해졌으나, "결과를 보고 기준을 조정했다"는 지적 자체는 타당하다 |
