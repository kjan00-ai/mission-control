...
- 날짜: 2026-09-07
- 상태: **v0.2 — 대안 C 채택(대표 승인 2026-09-07). 검증 5종 통과 · 적용 대기**
...
v0.2 기준으로 5종을 모두 재실행했다. 전부 통과.

**① 정규식 분리 왕복** (`split.js`) — 9개 대안 분리 → `|` 재결합이 원문과 byte-동일. T2 로 내리는 대안과 DENY 에 남는 대안의 차이는 **수량자 1글자(`*` ↔ `+`)** 뿐임을 문자 단위로 출력해 확인.

**② 스모크 38건** (`apply.js --check`, 쓰기 전 게이트) — PASS. 평문 11 + **접두형 7(v0.1 이 뚫었던 형태)** + 드라이런 2 + 나머지 파괴적 대안 13 + 룰 귀속 5.

**③ 고정 코퍼스 전수 대조** (`corpus-diff.js` — WSL + Windows 감사 로그 전부, 합성 케이스 없음):

```
코퍼스: 고유 명령 40,549 건 (총 실행 48,024 건)
[1] 강화(등급 상승)                    0 ✅
[2] 완화 7건 · 게이트 밖으로 1건
      ├ 실행 경로 이탈(구멍)            0 ✅
      └ 데이터 경로 완화(검토)          1
...
데이터 경로 완화 1건은 과거 감사 프로브다 — `for c in "git clean -fd" …; do echo "$c"; node risk-classify.js "$c"; done`. 위험 문자열이 **따옴표 안 인자**로만 있고 `node` 에 argv 로 넘어갈 뿐 실행되지 않는다. 오탐 제거가 맞다.

**④ 노출면 probe 26형태** (`layer-probe.js`) — 실행 경로 이탈 **0**.

| 군 | 형태 | 결과 |
|---|---|---|
| EXEC 15 | 단독·결합/분리 플래그·경로 인자·체인·`bash -c`·`sh -c`·`eval`·`xargs`·heredoc→bash·명령치환·서브셸·`node -e` | 전부 **T2 유지** |
| PREFIX 8 | `--git-dir`·`--work-tree`·`--exec-path`·`--namespace`·`-c` 2개·`-C`·`--no-pager`·env+롱옵션 | 전부 **DENY 유지** ★ v0.1 회귀 없음 |
| DATA 3 | AI 프롬프트 · 문서 heredoc · 인용 인자 | 2건 마찰 해소 |

**⑤ 현행 회귀 가드** — 적용 전 `74 pass, 0 fail` 정상(정책·시험이 한 트랜잭션이라 적용 시 함께 81건으로 올라간다).
...
**회귀 가드 한계** (L2 `0f537290`): v0.1 §6 의 "T1 로는 절대 내리지 않는다 — 시험 76건이 고정"은 **과장이었다.** 76건은 GATE 축 74 + 룰귀속 2 뿐이고, §3 에서 확인된 T1 이탈 형태를 **한 건도 고정하지 않았다.**

v0.2 는 시험을 74 → **81건**으로 올리고, 추가 7건을 전부 **룰 귀속**으로 넣는다. GATE 축(DENY/T3/T2 중 아무거나)으로는 T2 와 DENY 를 구분하지 못해 이 설계의 계약을 지키지 못하기 때문이다:

| 케이스 | 기대 룰 | 무엇을 고정하는가 |
|---|---|---|
| 평문 · 평문+경로 | `git-clean` | 마찰 해소가 실제로 T2 규칙으로 간다 |
| `--git-dir` · 롱옵션 2개 · `-c` 2개 · `-C` | `git-destructive` | **접두형이 DENY 를 떠나지 않는다** |
| `reset --hard` | `git-destructive` | 나머지 8개 대안 불변 |

이 시험이 v0.1 에 있었다면 접두형 구멍을 **자동으로 잡았다.** 앞으로 누가 `+` 를 `*` 로 되돌리면 시험이 깨진다.
...
**✅ 대표 판정: (C) 채택 · (B) 는 별건 (2026-09-07).** (C) 는 요청된 결과를 안전하게 내주고, (B) 가 완료되면 (C) 의 보상 통제(`+` 수량자)를 제거해 평문 (나) 로 수렴시킨다.

### 적용

```bash
node ~/p1c/candidates/gate-clean-t2/apply.js --apply       # 정책 + 시험 한 트랜잭션
node ~/.ai-bootstrap/gate-destructive.test.js              # 81/81 기대
node ~/.ai-bootstrap/maia-deploy.js                        # Windows 동기 (canonical=WSL)
```

적용 전 검증은 §6 ①~⑤ 로 이미 통과했다(`--check` 는 매 실행마다 스모크 38건을 다시 돌린다 — 적용 직전 상태로 한 번 더 확인된다).

롤백: `node ~/p1c/candidates/gate-clean-t2/apply.js --rollback` — 정책·시험 동시 복원(`*.bak-clean-t2`).
