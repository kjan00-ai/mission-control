...
corroborated 2(반영): `73efb577`(leakMinSamples 정본 누락 → policy 추가 + c6-leak precedence 보강), `66194db0`(readLeak 손상 JSON 실패모드 → try/catch parse 안전 + atomic write 명시). escalate 2(대표 판정, refuted): `7716c27b`(executed=0 measured 미핀고정 fail-open — gemini refute: leakPipelineLive false라 fail-closed; 실은 measured=executed≥minSamples 불변식이 가드), `d1d0bbf3`(활성화 기준 모호 — claude refute: executed≥leakMinSamples 정량+대표 책임+지연 무해). 미검증0·파싱실패0. 집계 `reviews/...-readleak-wire-l2-deepen-r3-20260630-160114.md`. 재검증 7/7 pass.
