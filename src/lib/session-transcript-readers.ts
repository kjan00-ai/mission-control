// Transcript readers for /api/sessions/transcript.
//
// Extracted from the route file because Next.js 16 forbids route files from
// exporting anything other than HTTP handlers + route config. These readers are
// pure (fs / sqlite / config) with no Next dependency; the route imports them and
// tests import them directly instead of via a route `__testables` export.
